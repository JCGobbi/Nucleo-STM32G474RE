...
with Inverter_ADC.Control; use Inverter_ADC.Control;
                           use Inverter_ADC;
with Inverter_Timer;       use Inverter_Timer;
...
package body StartUp is
   ...
   begin
      ...
      --  Open (False) or closed (True) loop for AC output
      Output_Loop_Gain := False;

      --  Initialize GPIO ports
      Initialize_GPIO;

      --  Select the AC frequency of the inverter
      if (Read_AC_Frequency) then --  60 Hz
      ...
      Initialized :=
         ...
         Inverter_Timer.Is_Initialized and
